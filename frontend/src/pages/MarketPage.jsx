...
export default function MarketPage({ id, account, refresh, onTrade, onError, onBack }) {
  ...
  return <>
    <button className="link-button" onClick={onBack}>← All markets</button><div className="market-heading"><span className={`category category-${instance.category}`}>{categories[instance.category]}</span><h1>{instance.title}</h1><p className="muted">{instance.data_mode === "simulated" ? "Simulated observations" : "Evidence from the published source"} · Times in Singapore (SGT)</p></div>
