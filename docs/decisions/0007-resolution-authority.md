...
The demo bus series now resolves through the external-resolver contract instead of administrator evidence, and the adapter the consequences section anticipated (the bus timing API needs a small adapter) is an endpoint the platform serves itself, backed by a live provider.

- `POST /api/v2/resolvers/ntu-bus` (`backend/src/resolver.rs`, `ntu_bus_answer`) accepts the fixed resolver request. The live data path comes first: the NTU Bus API provider (`provider/ntubus`, a standalone Python service wrapping the reverse-engineered Omnibus client) implements the same contract, polls every route's buses and pickup points, and answers Yes or No from recorded arrivals inside the bracket window. The adapter tries the provider a few times, relays its answer tagged `source: ntubus-live`, and falls back to the deterministic simulated feed (tagged `source: simulated-fallback`) only when the provider stays without a definitive answer, so development and CI work without the provider running. The recorded evidence carries the tag. Before a bracket's observation window ends, the answer is always pending.
- The fixed request became self-contained for integrators: it now carries the published `outcomes` (id and label), the instance `title`, and the `evidence_deadline_ms`, so a resolver can answer without a second lookup and knows when answers stop counting. Extra response fields are ignored.
- Demo seeding publishes one welfare series per scheduled campus line (Blue and Red daily, Green weekdays, Brown weekends, each on its published 07:30 to 23:00 span with real pickup points as stops; the recurring schedule gained ISO `active_days`), with the adapter URL built from the server's own `POLYNTU_BIND` loopback address. Databases seeded with older specs keep their old bus series but ended; the current specs take over, because published definitions are immutable.
- Resolver-authority instances never fall back to the simulated-evidence path the demo uses for administrator-authority markets, so a broken resolver surfaces as retries and a deadline void instead of being masked by automatic evidence. The adapter's own fallback to the deterministic feed is explicit and tagged in the evidence, not a mask.
