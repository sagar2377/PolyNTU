...
`.github/workflows/verify.yml` runs on pushes and pull requests using Ubuntu, PostgreSQL 17, Node 24, and the latest stable Rust toolchain (the dependency set, sqlx 0.9 in particular, requires at least Rust 1.94). A `changes` job detects which areas the commit touched, and each job runs only when its area changed: backend changes gate the Rust formatting, tests, clippy, and the `performance` job; frontend changes gate the npm install, lint, and build; any backend, frontend, or browser-test change also runs the `browser-e2e` job, which drives the real UI in headless Chrome over the DevTools protocol. Commits touching none of those areas skip every job. The `performance` job builds the release backend, starts it against the job's PostgreSQL service, and runs the throughput KPI benchmark as a regression gate; see [verification](verification.md#throughput-kpi-benchmark).
