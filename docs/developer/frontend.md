...
`PriceHistoryChart` (UC-19) draws one line per outcome plus a volume histogram pane for one instance, using the bucketed history endpoint. It derives the bucket size as the chart span divided by 180, clamped between one second and one hour: direct markets use a one-hour span (20-second buckets), while a series bracket uses its slot length as the span, so intervals up to three minutes clamp to the one-second minimum. The axis is a percent scale, so the 0 to 1 probabilities are fed multiplied by 100; a history of a single bucket (an untraded market) is extended with one same-valued bucket so the flat opening price renders as a line instead of an empty chart. It refetches whenever `reloadKey` changes, which the market page raises on every SSE-driven snapshot reload and on its five-second fallback poll, so the chart refreshes with every trade.
