...
- `RESOLUTION_KEY_ITERATIONS` (600,000) and `resolution_key_seed` derive the ed25519 seed of a creator's resolution signing key from the account password (ADR 0007 amendment): PBKDF2-HMAC-SHA256 over the password with the salt `polyntu.resolution.v1:{email}`, so holding the password is holding the key and any browser where the creator signs in can re-derive the keypair; only the resulting public key is published. The unit test `resolution_key_seed_matches_the_browser_derivation` pins the derivation to a WebCrypto known-answer vector (seed and public key) and rejects wrong passwords and wrong emails.
...
This owns the trade transaction in six statements: claim idempotency (replaying a committed response on conflict), lock the instance, read the database clock in a separate statement (a combined statement takes its snapshot when it starts, so a trade that waited on the row lock could wake with a stale close time), lock both accounts while reading balances and the position, insert the transfer, then apply the remaining writes (position upsert, inventory/version update, trade insert, idempotency response, outbox event, and `pg_notify`) in one data-modifying CTE before committing.
...
`BRACKET_RETENTION_MS` (24 hours) is how long a terminal recurring bracket outlives its evidence deadline. `purge_expired_brackets` runs on every worker tick: inside one transaction it sets the session-local `polyntu.purge = 'on'` flag and defers constraints (both revert at commit), selects up to 20 terminal recurring-series brackets whose evidence deadline passed more than 24 hours ago and whose reserve balance is zero, and deletes each with its outbox rows, positions, trades, settlement claims, and evidence. One-time markets are kept indefinitely, and so is the append-only accounting history: every ledger transfer is shared between two accounts, so drained reserve accounts and their ledger trails remain, as do admin audit and idempotency rows. A reserve that still holds units blocks the purge, so a settlement anomaly surfaces instead of vanishing. Purged instances are invalidated in the read-through cache after commit; a purge failure is logged and retried on the next tick.
...
`tick` closes due instances, spawns due series brackets, selects actionable rows, produces due simulator evidence, asks resolver-authority instances' external endpoints from their finalize window (ADR 0007: a valid answer becomes resolver evidence; pending, malformed, and unreachable sources retry on every tick until the published deadline voids the instance), settles independent instances concurrently in chunks of eight, purges terminal recurring brackets past the retention window in batches of 20 (a failure is logged and retried on the next tick), and then seeds future demos. Per-series spawn failures and per-instance evidence/settlement errors are logged so one failure cannot stall the others.
