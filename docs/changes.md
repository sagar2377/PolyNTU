...
## 20 September 2026: One market page per bracket, and the bus resolves through the NTU Bus API adapter

- The separate series page is gone. Every bracket's market page now carries its whole series: the schedule facts (interval, operating window, horizon, end date, fee, resolution authority, per-bracket liquidity, series state, criterion), the day-long probability view, the sibling bracket lists, and the creator's resolve controls. A series chip on the browse page opens one of the series' brackets (the first live one), and publishing opens the new market's page. The market page has a single back link returning to the list it was opened from (browse or the portfolio), so bracket-to-bracket navigation no longer stacks two layers of back, and the bracket being viewed is marked This bracket instead of a self-linking Trade button.
- The day-long probability view (UC-21) is redesigned as a vertical list of horizontal bars, longer meaning a higher first-outcome probability, with each bracket's time window, percentage, and traded volume. Slots show their actual implied probability, so an untraded series reads as a flat column of opening probabilities; the previous chart pinned settled slots to 0 or 1, which made a fresh 50/50 bus series look like it moved between 0% and 1%, and is removed together with `DayProbabilityChart`.
- The demo bus series now resolves through the external-resolver contract instead of administrator evidence: the platform serves its own NTU Bus API adapter at `POST /api/v2/resolvers/ntu-bus`, which answers simulated bus brackets from the deterministic feed once their observation window has ended (the live NTU Bus API feed is deferred work). Demo seeding builds the loopback endpoint from `POLYNTU_BIND`; databases seeded before the switch keep their old bus series but ended, and a fresh resolver-authority series takes over. Resolver-authority instances never fall back to simulated evidence, so a broken resolver is visible instead of masked, and a valid adapter answer that a demo clock jump made late is recorded as a replay instead of voiding the bracket, mirroring the simulated-evidence path; manual instances keep the hard deadline ([ADR 0007 amendment](decisions/0007-resolution-authority.md)). New integration test `bus_series_settles_through_the_ntu_bus_adapter` drives the real HTTP route end to end, including the clock-jump replay.
- The backend now has 14 unit tests, 38 integration tests, and 1 numerical test, all passing locally on rustc 1.98.1.
