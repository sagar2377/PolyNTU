-- Trading fees and market-creator attribution. Each trade's fee rides inside
-- the single trader/reserve transfer and is recorded per trade; at settlement
-- the accumulated pot leaves the reserve as explicit 'fee' transfers, split
-- between the platform treasury and the instance's recorded creator.
ALTER TABLE instances ADD COLUMN creator_account_id TEXT REFERENCES accounts(id);
ALTER TABLE trades ADD COLUMN fee_micros BIGINT NOT NULL DEFAULT 0 CHECK (fee_micros >= 0);
ALTER TABLE ledger_transfers DROP CONSTRAINT ledger_transfers_kind_check;
ALTER TABLE ledger_transfers ADD CHECK (kind IN ('issuance','grant','subsidy','buy','sell','resolution','release','fee'));

-- Creator attribution is fixed at publication, like every other published rule.
...
BEGIN
    IF ROW(NEW.template_id,NEW.category,NEW.title,NEW.resolution_criterion,NEW.rule,NEW.outcomes,
           ...
           NEW.finalize_after_ms,NEW.evidence_deadline_ms,NEW.liquidity_units,NEW.reserve_account_id,NEW.creator_account_id)
       IS DISTINCT FROM
       ROW(OLD.template_id,OLD.category,OLD.title,OLD.resolution_criterion,OLD.rule,OLD.outcomes,
           ...
           OLD.finalize_after_ms,OLD.evidence_deadline_ms,OLD.liquidity_units,OLD.reserve_account_id,OLD.creator_account_id) THEN
